// ============================================
// WASION Safety Observer - Service Worker v4
    ...
    try {
        ...
        try {
            ...
            return new Response(JSON.stringify({
                success: true,
                offline: true,
                message: 'Observación guardada. Se enviará cuando vuelvas a tener conexión.'
            }), {
                status: 202,
                headers: { 'Content-Type': 'application/json' }
            });
        ...
        for (const item of queue) {
            try {
                ...
                if (response.ok) {
                    ...
                    await notifyUser('Observación sincronizada', 
                        'Tu observación guardada offline ha sido enviada correctamente.');
                } else {
                    console.error(`[SW] ❌ Error sincronizando ${item.id}:`, response.status);
                }
            } catch (error) {
...
console.log('[SW] 🚀 Service Worker v4 cargado - Modo Offline Real + Background Sync');
