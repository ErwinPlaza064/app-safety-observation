...
export default function ObservationDetailsModal({
    ...
    const handleShare = async () => {
        setIsSharing(true);
        try {
            const response = await axios.post(route('observations.share', observation.id));
            const { url, expires_in_days } = response.data;
            
            await navigator.clipboard.writeText(url);
            setCopied(true);
            setExpiryInfo(`${expires_in_days} días`);
            setTimeout(() => {
                setCopied(false);
                setExpiryInfo(null);
            }, 3000);
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="grid grid-cols-1 gap-3">
                                        <div className="p-3 bg-gray-50 dark:bg-gray-900/40 border border-gray-100 dark:border-gray-700 rounded-xl flex items-center gap-3">
                                            ...
                                            <div>
                                                <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Planta</p>
                                ...
                                <>
                                    {canShare && (
                                        <button
                                            onClick={handleShare}
                                            disabled={isSharing}
                                            className="px-5 py-2 text-xs font-black tracking-widest text-indigo-600 dark:text-indigo-400 uppercase transition-all bg-indigo-50 dark:bg-indigo-900/30 border-2 border-indigo-100 dark:border-indigo-800/50 rounded-xl hover:bg-indigo-100 dark:hover:bg-indigo-800/40 shadow-sm flex items-center gap-2 group disabled:opacity-50 flex-1 sm:flex-none justify-center"
                                            title="Compartir enlace temporal (7 días)"
                                        >
                                            {isSharing ? (
                                            ...
                                            <span>
                                                {copied ? (
                                                    <>Copiado <span className="opacity-60 ml-0.5">· {expiryInfo}</span></>
                                                ) : (
                                                    'Compartir'
                                                )}
                                            </span>
